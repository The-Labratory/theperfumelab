    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display tracking-wider">Shopping Cart</SheetTitle>
          <SheetDescription>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium truncate text-sm font-body">{item.product.node.title}</h4>
                        <p className="text-xs text-muted-foreground">{item.selectedOptions.map(o => o.value).join(' • ')}</p>
                        <p className="font-semibold text-sm font-display">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
